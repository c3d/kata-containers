...
// Define tags for the trace span
var ociTracingTags = map[string]string{
	"source":    "runtime",
	"package":   "katautils",
	"subsystem": "oci",
}

var kataUtilsLog = logrus.WithField("source", "katautils")
...
func AddContainerIDMapping(ctx context.Context, containerID, sandboxID string) error {
	span, ctx := katatrace.Trace(ctx, kataUtilsLog, "AddContainerIDMapping", ociTracingTags)
	defer span.End()
...
func DelContainerIDMapping(ctx context.Context, containerID string) error {
	span, ctx := katatrace.Trace(ctx, kataUtilsLog, "DelContainerIDMapping", ociTracingTags)
	defer span.End()
